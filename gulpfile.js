...
const getPageData = () => {
   ...
   const getPageData = {
      ...
      includes: includes,
      image_tag: (filename) =>
         `<img alt=${code(filename)} class=image-${code(filename)} src=${filename}>`,
...
const task = {
   ...
   buildFonts() {
      return gulp.src('source/fonts/**/*.+(ttf|woff|woff2)')
         ...
         .pipe(gulp.dest('build/stag/fonts'));
      },
   buildImages() {
      return gulp.src('source/images/**/*')
         .pipe(gulp.dest('build/dev/images'))
         .pipe(gulp.dest('build/stag/images'));
      },
   buildJs() {
      const config = readIndexYml();
   ...
   runServer() {
      ...
      gulp.watch('build/dev/**/*').on('change', server.reload);
      },
   showPaths() {
      console.log('Source input (markdown):  ', chalk.green(path.resolve('source')));
      console.log('Regular output (HTML):    ', chalk.white(path.resolve('build/dev')));
      console.log('Minified output (HTML):   ', chalk.white(path.resolve('build/stag')));
      console.log('Production output (HTML): ', chalk.white(path.resolve('build/prod')));
      return gulp.src('source/*.html');
      },
   publishToDocs() {
      fs.mkdirSync('docs', { recursive: true });
      fs.writeFileSync('docs/CNAME', 'node-slate.js.org\n');
      return gulp.src('build/stag/**/*')
         .pipe(gulp.dest('docs'));
      },
   };
